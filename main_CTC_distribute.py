...
def Train():
    ...
    stratedy = tf.distribute.MirroredStrategy(devices=["device:GPU:0", "device:GPU:1"])
    with stratedy.scope():
    ...
    def _train():
        for _ in tf.range(1000):
            stratedy.experimental_run_v2(train_CTC_supervised, args=(next(iter_feature_train), dataset_train, model, optimizer))
...
# @tf.function(experimental_relax_shapes=True)
def train_CTC_supervised(m, dataset_train, model, optimizer):
    uttids, x = m
    labels = dataset_train.get_attrs('trans', uttids.numpy())
