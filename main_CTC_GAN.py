...
def train():
    ...
    optimizer_G = tf.keras.optimizers.Adam(args.opti.G.lr, beta_1=0.1, beta_2=0.5)
    ...
    while step < 99999999:
        ...
        if phrase_ctc:
            ...
            _trans = dataset_train_supervise.get_attrs('trans', _uttids.numpy())
            train_GAN_G(x, _x, _trans, G, D, optimizer_G, args.model.D.max_label_len, args.lambda_supervise)

        num_processed += len(x)
        progress = num_processed / args.data.train_size
        if step % 10 == 0:
            if phrase_ctc:
                print('loss_G_ctc: {:.3f}\tbatch: {}\tused: {:.3f}\t {:.3f}% iter: {}'.format(
                       loss_G_ctc, x.shape, time()-start, progress*100.0, step))
                with writer.as_default():
                    tf.summary.scalar("costs/loss_G_supervise", loss_G_ctc, step=step)
            else:
                print('loss_GAN: {:.3f}|{:.3f}|{:.3f}\tbatch: {}|{}\tused: {:.3f}\t {:.3f}% iter: {}'.format(
                       loss_D_fake, loss_D_real, gp, x.shape, text.shape, time()-start, progress*100.0, step))
...
def train_GAN_G(x, _x, _ｙ, G, D, optimizer_G, len_D, lambda_supervise):
    params_G = G.trainable_variables[:-4] # onl;y the final fc
    with tf.GradientTape(watch_accessed_variables=False) as tape_G:
        tape_G.watch(params_G)
...
def train_D(x, P_Real, mask_real, G, D, optimizer_D, lambda_gp, len_D):
    ...
    P_Real *= tf.cast(mask_real, tf.float32)[:, :, None]
